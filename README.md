...
To perform a parameter sweep, you must pass values for the `--sweep` and `--sweep-values` arguments.
```bash
python run.py --config example_config.json --sweep size --sweep-values 100 200 300 400 500 --runs 15
```
The command above will sweep the simulation's size parameter (i.e. the total number of vibrating particles) over the values 100, 200, 300, 400, and 500. It will perform 15 runs **for each** value in `--sweep-values`.
...
```bash
python run.py --config example_config.json --sweep high_freq --sweep-values 8 9 10 --runs 2 --label "frequency-sweep"
```
